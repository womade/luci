...
function index()
	...
	local page
	page = entry({ "admin", "nas", "aliyundrive-webdav" }, alias("admin", "nas", "aliyundrive-webdav", "client"),
		_("AliyunDrive WebDAV"), 10) -- 首页
	page.dependent = true
	page.acl_depends = { "luci-app-aliyundrive-webdav" }

	entry({ "admin", "nas", "aliyundrive-webdav", "client" }, cbi("aliyundrive-webdav/client"), _("Settings"), 10).leaf = true -- 客户端配置
	entry({ "admin", "nas", "aliyundrive-webdav", "log" }, form("aliyundrive-webdav/log"), _("Log"), 30).leaf = true -- 日志页面

	entry({ "admin", "nas", "aliyundrive-webdav", "status" }, call("action_status")).leaf = true -- 运行状态
	entry({ "admin", "nas", "aliyundrive-webdav", "logtail" }, call("action_logtail")).leaf = true -- 日志采集
	entry({ "admin", "nas", "aliyundrive-webdav", "qrcode" }, call("action_generate_qrcode")).leaf = true -- 生成扫码登录二维码地址和参数
	entry({ "admin", "nas", "aliyundrive-webdav", "query" }, call("action_query_qrcode")).leaf = true -- 查询扫码登录结果
	entry({ "admin", "nas", "aliyundrive-webdav", "invalidate-cache" }, call("action_invalidate_cache")).leaf = true -- 清除缓存
end
